...
export default function ElevenLabsWaveform({ status, isConnected }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-1.5 select-none w-full">
      {/* Compact 36-bar High-Density Waveform */}
      <div className="flex items-center justify-center gap-[3px] h-10 w-full max-w-sm px-2">
        {Array.from({ length: barCount }).map((_, index) => {
          ...
          const baseHeight = 4 + curveFactor * 14;
            ...
            <div
              ...
              className={`w-[2.5px] rounded-full transition-all duration-200 ${
                !isConnected
                  ? 'bg-zinc-800'
                  : isSpeaking
                  ? 'bg-gradient-to-t from-violet-600 via-fuchsia-400 to-white shadow-[0_0_8px_rgba(168,85,247,0.7)]'
                  : isListening
                  ? 'bg-gradient-to-t from-emerald-600 via-teal-300 to-white shadow-[0_0_8px_rgba(16,185,129,0.7)]'
                  : isThinking
                  ? 'bg-gradient-to-t from-amber-500 to-orange-300 shadow-[0_0_6px_rgba(245,158,11,0.5)]'
                  : 'bg-zinc-700'
              }`}
              style={{
                height: !isConnected
                  ? '6px'
                  : isActive
                  ? `${Math.max(6, baseHeight * 1.5)}px`
                  : isThinking
                  ? `${Math.max(5, (Math.sin(index * 0.8) + 1) * 8 + 4)}px`
                  : `${Math.max(5, baseHeight * 0.4)}px`,
                animation: isActive
